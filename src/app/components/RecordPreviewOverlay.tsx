...
export function RecordPreviewOverlay({
  ...
  const startFlow = useCallback(async () => {
    ...
    // Prime the canvas with an opaque frame BEFORE capturing.
    ctx.fillStyle = bg;
        ...
        <div style={{
          ...
          {phase === "countdown" && (
            <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", pointerEvents: "none" }}>
              <span style={{ fontFamily: FONT_SERIF, fontSize: Math.round(frameHeight * 0.55) + "px", lineHeight: 1, color: dark ? "rgba(240,232,220,0.65)" : "rgba(42,42,40,0.5)" }}>
                {countdown}
              </span>
            </div>
          )}
