...
// ── Fixed-grid layout constants (used by PlaygroundNew) ──────────────────────
const COL_W              = 363;
const COL_GAP            = 100;
const ROW_GAP            = 100;
const GRID_EXPLORE_SCALE = 0.55;
const GRID_W             = 5 * COL_W + 4 * COL_GAP; // 2215
const GRID_H             = 1544;

interface GSlot { t: string; rot?: number }

const GTOOLS: Record<string, { h: number; borderRadius: string; bgL: string; bgD: string; vL: string; vD: string; label: string; href: string }> = {
  spiral:    { h: 120, borderRadius: "200px",             bgL: "#eef7ff", bgD: "#242c38", vL: "in-a-spiral-light",         vD: "in-a-spiral-dark",         label: "...in a spiral",             href: "/in-a-spiral" },
  asym:      { h: 174, borderRadius: "40px 4px 40px 4px", bgL: "#f6f8ed", bgD: "#2f2836", vL: "visible-corrections-light", vD: "visible-corrections-dark", label: "...with visible corrections", href: "/visible-corrections" },
  blind:     { h: 190, borderRadius: "100px",              bgL: "#ecfaf0", bgD: "#222d26", vL: "blind-then-witness-light",  vD: "blind-then-witness-dark",  label: "...blind & then witness",    href: "/blind-then-witness" },
  rect:      { h: 235, borderRadius: "4px",                bgL: "#fff4f6", bgD: "#37262d", vL: "off-the-grid-light",        vD: "off-the-grid-dark",        label: "...off the grid",            href: "/off-the-grid" },
  circle:    { h: 363, borderRadius: "200px",              bgL: "#fef8ee", bgD: "#3e3e3e", vL: "without-stopping-light",   vD: "without-stopping-dark",    label: "...without stopping",        href: "/without-stopping" },
  uninvited: { h: 182, borderRadius: "4px",                bgL: "#eaf8f5", bgD: "#1f2f29", vL: "uninvited-thoughts-light",  vD: "uninvited-thoughts-dark",  label: "...uninvited thoughts",      href: "/uninvited-thoughts" },
};

const GRID_COLS: GSlot[][] = [
  [{ t: "spiral" }, { t: "asym" }, { t: "blind", rot: -6.42 }, { t: "rect" }, { t: "circle" }],
  [{ t: "asym", rot: -4 }, { t: "rect", rot: 4.64 }, { t: "spiral", rot: -4.37 }, { t: "blind", rot: 3.39 }, { t: "circle" }],
  [{ t: "asym" }, { t: "blind", rot: 5.27 }, { t: "circle" }, { t: "uninvited", rot: -4 }, { t: "rect" }],
  [{ t: "asym", rot: 2.17 }, { t: "blind" }, { t: "circle" }, { t: "spiral", rot: -3.91 }, { t: "rect", rot: 5 }],
  [{ t: "rect", rot: 4.75 }, { t: "blind" }, { t: "circle" }, { t: "asym", rot: 4.75 }, { t: "spiral" }],
];

function GridToolShape({ t: tKey, rot }: GSlot) {
  const theme    = useContext(ThemeContext);
  const dark     = useContext(DarkContext);
  const t        = GTOOLS[tKey];
  const bg       = dark ? t.bgD : t.bgL;
  const video    = dark ? t.vD  : t.vL;
  const videoRef = useRef<HTMLVideoElement>(null);
  const [hovered, setHovered] = useState(false);
  ...
  useEffect(() => {
    ...
    play();
    if (el.readyState < 2) el.addEventListener("canplay", play, { once: true });
  }, [video]);
  ...
  return (
    ...
    >
      ...
      <span style={{
        ...
        fontFamily: FONT_SANS, fontSize: 17, color: dark ? "#aaa" : "#555",
        opacity: hovered ? 1 : 0,
        transition: "opacity 120ms ease",
...
function HeroHeading({ DE, theme }: { DE: boolean; theme: Theme }) {
  ...
  return (
    <h1 style={{ margin: 0, fontFamily: FONT_CMP_SERIF, fontSize: 34, lineHeight: "42px", fontWeight: 400, color: theme.headline, textAlign: "center", whiteSpace: "nowrap", animation: "_heroIn 1s ease-out both" }}>
      <style>{`
...
// ── Fixed-grid explore version (the "festes Raster" layout) ────────────────
// Clicking "Explore" zooms out slightly and reveals the user's own tools in a
// fixed set of slots around the hero. The endless blue-noise field lives in
// PlaygroundField (route /tool-collection/field).
      ...
      <main style={{ minHeight: "100vh", height: "100vh", width: "100vw", overflowX: "hidden", overflowY: exploreMode ? "hidden" : "auto", position: "relative", backgroundColor: theme.bg, backgroundImage: exploreMode ? "none" : theme.dotGrid, backgroundSize: "42px 42px", color: theme.text, fontFamily: FONT_SANS, WebkitOverflowScrolling: "touch" }}>
        ...
        {!exploreMode && (
          <button
            ...
            title={DE ? "Zur Raster-Ansicht wechseln" : "Switch to grid view"}
          >
            ← {DE ? "Raster-Ansicht" : "Grid view"}
          </button>
        )}
        {exploreMode && (
          <button
